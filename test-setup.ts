...
// passes `--timeout=${FREE_TEST_TIMEOUT_MS}` to every shard), NOT here.
// Calling setDefaultTimeout() from a preload looks like it works and does
// not: bun resets the default for each test FILE, so a preload call only ever
// governs the first file in the run. Measured on bun 1.3.13 — one file with a
// 6s test passes, the same test behind any other file times out at 5000ms.
//
// Why well above bun's 5s: the unit of work in large parts of this suite is
// "launch Chromium" or "spin a git repo", each about 0.9s on an idle machine,
// and the gate runs hundreds of files while this box may also be running a
// second gate in a sibling worktree. Cases that went red purely on the clock:
