/**
 * Source-text invariants for the sidebar surfaces that still exist.
 *
 * The sidebar's primary surface is the Terminal pane (interactive claude
 * PTY). Activity / Refs / Inspector are debug overlays behind the footer's
 * `debug` toggle. See docs/designs/SIDEBAR_MESSAGE_FLOW.md.
 *
 * The chat-queue path (one-shot `claude -p` via sidebar-agent.ts, the
 * /sidebar-chat, /sidebar-command, /sidebar-agent/*, /sidebar-tabs
 * endpoints, the chat bubbles, the browser tab bar and the model router)
 * was ripped in v1.14.0.0. Every describe block that asserted it was
 * deleted here rather than skipped — a skipped test claiming to guard a
 * surface that no longer exists is the same lie as a red one.
 *
 * What remains, and what still covers it:
 * - Browser-manager tab bookkeeping (switchTab focus semantics, page
 *   add/remove tracking).
 * - The quick-action toolbar (cleanup / screenshot / cookies), which now
 *   injects into the live PTY via window.gstackInjectToTerminal.
 * - Inspector + CSP-fallback picker wiring in the extension.
 * - Welcome page, sidebar auto-open, the arrow-hint signal chain, auth
 *   race prevention, BROWSE_NO_AUTOSTART.
 */
...
describe('browser→sidebar tab state', () => {
  const bmSrc = fs.readFileSync(path.join(ROOT, 'src', 'browser-manager.ts'), 'utf-8');
  ...
  test('background.js pushes tab state on every tab event', () => {
    // The chat path's per-tab URL sync is gone. The live path is
    // background.js → pushTabState → sidepanel.js → sidepanel-terminal.js,
    // which forwards over the PTY WebSocket so terminal-agent.ts can write
    // <stateDir>/active-tab.json + tabs.json for claude to read.
    ...
    expect(bgSrc).toContain("pushTabState('activated')");
    expect(bgSrc).toContain("pushTabState('created')");
    expect(bgSrc).toContain("pushTabState('removed')");
    expect(bgSrc).toContain("type: 'browserTabState'");
  });

  test('terminal pane forwards tab state over the PTY WebSocket', () => {
    ...
    // Middle hop: sidepanel.js re-dispatches the runtime message as a DOM event
    expect(spSrc).toContain("msg.type === 'browserTabState'");
    expect(spSrc).toContain("new CustomEvent('gstack:tab-state'");
    // Last hop: the terminal pane sends it down the live PTY WebSocket
    expect(termSrc).toContain("'gstack:tab-state'");
    expect(termSrc).toContain("type: 'tabState'");
  });
});
...
describe('CSP fallback basic picker', () => {
  ...
  test('content.js contains CSSOM iteration guarded against cross-origin sheets', () => {
    ...
    // Reading .cssRules on a cross-origin sheet throws a SecurityError
    // (a DOMException). The per-sheet catch is what keeps one foreign
    // stylesheet from aborting the whole walk.
    expect(contentSrc).toContain('catch (e) { if (!(e instanceof DOMException)) throw e; }');
  });
...
describe('cleanup and screenshot buttons', () => {
  ...
  test('cleanup button injects a prompt into the live PTY', () => {
    const cleanupFn = js.slice(
      js.indexOf('async function runCleanup('),
      js.indexOf('async function runScreenshot('),
    );
    // The old path POSTed to /sidebar-command, which spawned a one-shot
    // claude -p. That endpoint is gone; the prompt now goes to the running
    // REPL so the user watches it happen in the Terminal pane.
...
// ─── No focus stealing when the server pins a tab ───────────────

describe('handleCommand does not steal browser focus', () => {
...
describe('PTY-injected cleanup prompt', () => {
  const js = fs.readFileSync(path.join(ROOT, '..', 'extension', 'sidepanel.js'), 'utf-8');
  const wcSrc = fs.readFileSync(path.join(ROOT, 'src', 'write-commands.ts'), 'utf-8');

  // The cleanup button hands claude a prompt, not a selector list. These
  // tests pin the parts of that prompt that carry the behaviour: run the
  // deterministic sweep first, then look at what survived and hide it,
  // without stripping the page's own identity.
  const cleanupFn = js.slice(
    js.indexOf('async function runCleanup('),
    js.indexOf('async function runScreenshot('),
  );

  test('prompt goes through the injection scan before reaching the PTY', () => {
    const scanIdx = cleanupFn.indexOf('gstackScanForPTYInject');
    const injectIdx = cleanupFn.indexOf('gstackInjectToTerminal');
    expect(scanIdx).toBeGreaterThan(0);
    expect(injectIdx).toBeGreaterThan(scanIdx);
    // A BLOCK verdict must not fall through to the injection
    expect(cleanupFn).toContain("verdict?.verdict === 'BLOCK'");
  });

  test('prompt runs the deterministic sweep first', () => {
    expect(cleanupFn).toContain('$B cleanup --all');
  });

  test('prompt tells claude to snapshot and inspect what survived', () => {
    expect(cleanupFn).toContain('$B snapshot -i');
    expect(cleanupFn).toContain('identify any remaining');
  });

  test('prompt names the clutter categories to look for', () => {
    for (const category of [
      ...
      expect(cleanupFn).toContain(category);
    }
  });

  test('prompt preserves site identity', () => {
  ...
  test('prompt unlocks scrolling', () => {
    expect(cleanupFn).toContain('unlock scrolling');
  });

  test('prompt hides survivors via $B eval', () => {
    expect(cleanupFn).toContain('$B eval');
  });

  test('loading state clears on a timer (injection is fire-and-forget)', () => {
...
describe('sidebar arrow hint hide flow (4-step signal chain)', () => {
  ...
  test('step 1: sidepanel sends sidebarOpened message on connect', () => {
    ...
    // Must fire from updateConnection, after setConnState('connected') —
    // scoped to the function body, not a fixed character window.
    const fnStart = spSrc.indexOf('function updateConnection(');
    expect(fnStart).toBeGreaterThan(-1);
    const fnEnd = spSrc.indexOf('\n}', fnStart);
    const connectFn = spSrc.slice(fnStart, fnEnd);
    expect(connectFn).toContain("setConnState('connected')");
...
describe('BROWSE_NO_AUTOSTART (sidebar headless prevention)', () => {
  ...
  // sidebar-agent.ts was ripped; terminal-agent.ts spawns the claude PTY
  // now and owns the same env contract.
  const agentSrc = fs.readFileSync(path.join(ROOT, 'src', 'terminal-agent.ts'), 'utf-8');

  // Both functions take arguments, so match on the declaration prefix —
  // an exact `()` signature silently resolved to -1 and sliced an empty
  // string, which made this block assert nothing.
  ...
  test('cli.ts checks BROWSE_NO_AUTOSTART before starting a new server', () => {
    expect(ensureServerStart).toBeGreaterThan(-1);
  ...
  test('terminal-agent.ts sets BROWSE_NO_AUTOSTART=1 for the claude it spawns', () => {
  ...
  test('BROWSE_NO_AUTOSTART check happens before lock acquisition', () => {
    ...
    const noAutoStart = ensureServerFn.indexOf('BROWSE_NO_AUTOSTART');
    const lockAcquisition = ensureServerFn.indexOf('acquireServerLock()');
...
describe('idle timeout behavior (server.ts)', () => {
  ...
  test('PTY lease refresh resets the idle timer', () => {
    // /sidebar-command was the old keepalive. The live one is
    // /internal/lease-refresh, which terminal-agent.ts calls on its 25s
    // cycle — an in-use sidebar terminal must keep the daemon alive.
    const idx = serverSrc.indexOf("url.pathname === '/internal/lease-refresh'");
    expect(idx).toBeGreaterThan(-1);
    const handler = serverSrc.slice(idx, idx + 1200);
    expect(handler).toContain('resetIdleTimer');
  });
});
