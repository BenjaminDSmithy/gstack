...
```bash
export GSTACK_PR_PREP_REPORT="${GSTACK_PR_PREP_REPORT:-/tmp/ship-pr-prep-$(git rev-parse --show-toplevel | shasum | cut -c1-8).json}"
...
rm -f "$GSTACK_PR_PREP_REPORT"
echo "PR_PREP_REPORT: $GSTACK_PR_PREP_REPORT"
```
...
- Write pr-prep's machine-readable report to `$GSTACK_PR_PREP_REPORT`
...
```bash
if [ ! -f "$GSTACK_PR_PREP_REPORT" ]; then
  echo "[ship] pr-prep produced no report — continuing (audit not run)" >&2
else
  PR_PREP_WORST=$(jq -r '.worst // "CLEAN"' "$GSTACK_PR_PREP_REPORT" 2>/dev/null || echo CLEAN)
  jq -r '.summary // empty' "$GSTACK_PR_PREP_REPORT" 2>/dev/null || true
...
Note: `$GSTACK_PR_PREP_REPORT` is read again in Step 19 (PR body
assembly) to surface SIBLING / OVERLAP findings as a collapsed
"Upstream context" section in the PR body. SIBLING context
